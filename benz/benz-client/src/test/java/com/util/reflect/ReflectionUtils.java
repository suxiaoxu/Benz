...
public class ReflectionUtils {
	 ...
	 * cl 可为空，为空时取系统类加载器。 返回值一定不为 null。
	 */
	public static ClassPathResource[] getClassPathResources(String dirPath,
	 ...
	 *            这些包下面的类会被扫描
	 * @param classLoaders
	 *            用来加载 basePackages 下面类的类加载器
	 * @return 能够扫描到的类
	 */
	public static Class<?>[] getAllClassPathClasses(String[] basePackages,
			ClassLoader... classLoaders) throws IOException {
		if (classLoaders.length <= 0) {
			classLoaders = new ClassLoader[] { Thread.currentThread()
					.getContextClassLoader() };
		}
		List<Class<?>> classes = Lists.newArrayListWithCapacity(100);
		for (ClassLoader classLoader : classLoaders) {
			for (String basePackage : basePackages) {
				classes.addAll(Lists.newArrayList(ReflectionUtils.getClasses(
						basePackage, classLoader)));
			}
		}
	...
	/**
	 * 使用 cl 指定的类加载器递归加载 packageName 指定的包名下面的所有的类。不会返回 null。 cl
	 * 为空时使用系统类加载器。返回值一定不为 null。返回值中不包含类路径中的内部类。
	 */
	public static Class<?>[] getClasses(String packageName, ClassLoader cl)
			throws IOException {
		if (cl == null) {
			cl = Thread.currentThread().getContextClassLoader();
		}
		ClassPathResource[] resources = getClassPathResources(
				StringUtils.replace(packageName, ".", "/"), cl);
	...
	private static Class<?> resolveClass(ClassLoader cl,
	 ...
	 * @throws IOException
	 */
	private static URL[] getRoots(String dirPath, ClassLoader cl)
			throws IOException {
	...
	private static Collection<ClassPathResource> doFindPathMatchingFileResources(
			URL rootUrl, String dirPath) throws IOException {
	...
	private static Collection<ClassPathResource> doFindMatchingFileSystemResources(
			File rootDir, String dirPath) throws IOException {
		...
		Set<ClassPathResource> result = new LinkedHashSet<ClassPathResource>(
				allFiles.size());
		for (File file : allFiles) {
			String absolutePath = file.getAbsolutePath();
			URL url = new URL("file:///" + absolutePath);
			String classPathPath = absolutePath.substring(classPathRoot
					.length());
	...
	private static Collection<ClassPathResource> doFindPathMatchingJarResources(
		...
		try {
			...
			for (Enumeration<JarEntry> entries = jarFile.entries(); entries
				...
				if (entryPath.startsWith(rootEntryPath)) {
					...
					String classPathPath = applyRelativePath(rootEntryPath,
							relativePath);
					result.add(new ClassPathResource(new URL(newPath),
							classPathPath));
				}
			}
			return result;
